...
function useSecondsAgo(anchorTime: Date | null) {
  ...
  useEffect(() => {
    ...
    const tick = () =>
      setSeconds(Math.floor((Date.now() - anchorTime.getTime()) / 1000));
    tick();
...
function SensorCard({
  ...
  const sensorStyles: Record<
    string,
    { bg: string; border: string; fg: string }
  > = {
    temp: {
      bg: "bg-sensor-temp-bg",
      border: "border-sensor-temp-border",
      fg: "text-sensor-temp-fg",
    },
    humid: {
      bg: "bg-sensor-humid-bg",
      border: "border-sensor-humid-border",
      fg: "text-sensor-humid-fg",
    },
    light: {
      bg: "bg-sensor-light-bg",
      border: "border-sensor-light-border",
      fg: "text-sensor-light-fg",
    },
    rain: {
      bg: "bg-sensor-rain-bg",
      border: "border-sensor-rain-border",
      fg: "text-sensor-rain-fg",
    },
    wind: {
      bg: "bg-sensor-wind-bg",
      border: "border-sensor-wind-border",
      fg: "text-sensor-wind-fg",
    },
    pressure: {
      bg: "bg-sensor-pressure-bg",
      border: "border-sensor-pressure-border",
      fg: "text-sensor-pressure-fg",
    },
    vpd: {
      bg: "bg-sensor-vpd-bg",
      border: "border-sensor-vpd-border",
      fg: "text-sensor-vpd-fg",
    },
    soil: {
      bg: "bg-sensor-soil-bg",
      border: "border-sensor-soil-border",
      fg: "text-sensor-soil-fg",
    },
  ...
  return (
    <Link href={href}>
      <Card
        className={`${style.bg} ${style.border} ${className} shadow-sm border cursor-pointer hover:shadow-md hover:brightness-95 transition-all h-full`}
      >
        <CardHeader className="flex flex-row items-center justify-between pb-1 pt-3 px-3">
          <div className="flex flex-col">
            <div className="flex items-center gap-1">
              <CardTitle className="font-semibold text-muted-foreground uppercase tracking-wide leading-none">
                {title}
              </CardTitle>
              {type === "vpd" && <VpdInfoButton />}
            </div>
            {SHOW_TOR && dbField && (
              <span className="text-muted-foreground/60 mt-1 uppercase">
                {dbField}
              </span>
            )}
          </div>
        ...
        <CardContent className="px-3 pb-2">
          <div className="flex items-baseline gap-2 flex-wrap">
            <div
              className={`text-xl leading-none tabular-nums font-bold ${style.fg}`}
            >
              {value != null
                ? `${typeof value === "number" && !Number.isInteger(value) ? value.toFixed(1) : value}`
                : "—"}
              <span className="font-bold ml-1">{unit}</span>
            </div>
            {vpdStatus && (
              <Badge
                ...
                className={
                  vpdStatus === "เหมาะสม"
                    ? "bg-green-600 text-white hover:bg-green-700 border-none px-1.5 h-5 text-xs gap-1"
                    : vpdStatus === "ต่ำ"
                      ? "bg-blue-600 text-white hover:bg-blue-700 border-none px-1.5 h-5 text-xs gap-1"
          ...
          <span className="inline-flex items-center gap-1 mt-1.5 text-[12px] text-muted-foreground/90">
            <BarChart2 className="h-3 w-3" /> แตะเพื่อดูประวัติย้อนหลัง
          </span>
        </CardContent>
      </Card>
    </Link>
...
function WindCombinedCard({
  ...
  return (
    <Card className="bg-sensor-wind-bg border-sensor-wind-border shadow-sm border h-full">
      <CardHeader className="flex flex-row items-center justify-between pb-1 pt-3 px-3">
        <div className="flex flex-col">
          <CardTitle className="font-semibold text-muted-foreground uppercase tracking-wide leading-none">
            ลม
        ...
        <Wind
          className="h-4 w-4 text-sensor-wind-fg opacity-80"
          aria-hidden="true"
        />
      </CardHeader>
      <CardContent className="px-3 pb-2">
        <div className="flex items-center gap-4">
          {/* Left: rows — เหลือแค่ 2 แถว */}
          <div className="flex-1 flex flex-col">
            <div className="flex items-center justify-between py-1.5 border-b border-sensor-wind-border/30">
              <span className="text-sm font-medium text-foreground">ลม</span>
            ...
            <div className="flex items-center justify-between py-1.5">
              ...
              <div className="text-right">
                <div className="text-sm text-muted-foreground">
                  {deg != null ? `${Math.round(c.deg)}° ${c.label}` : "—"}
                </div>
                {deg != null && (
          ...
          {/* Right: arrow */}
          <div className="shrink-0 w-[32px] h-[32px] flex items-center justify-center">
            {deg != null ? (
              <svg
                viewBox="0 0 24 24"
                className="w-full h-full text-sensor-wind-fg"
                style={{
...
function condIcon(code: number | null, source?: string): string {
  ...
  if (source !== "openmeteo") {
    // TMD cond 1-8
    const tmd: Record<number, string> = {
      1: "☀️",
      2: "🌤️",
      3: "⛅",
      4: "☁️",
...
function HourlyForecastCard({
  ...
  const condLabel =
    currentSlot?.weatherCode != null
      ? (TMD_COND_LABEL[currentSlot.weatherCode] ?? "")
      : "";
    ...
    <Card className="shadow-sm border overflow-hidden col-span-full bg-sky-50 border-sky-100">
      <CardContent className="p-0">
        <div className="overflow-x-auto scrollbar-none flex-1 flex items-center">
          ...
          <div className="flex flex-col justify-center items-center text-center px-5 pt-4 pb-3 md:w-64 md:shrink-0 md:border-r md:border-sky-100 gap-0.5">
            <div className="text-7xl font-thin leading-none tracking-tighter text-sky-600">
              {currentSlot?.temperature != null
                ? `${currentSlot.temperature}°`
                : "—"}
            </div>
            <div className="sm:text-[16px] font-normal text-slate-500 mt-2">
              {condIcon(currentSlot?.weatherCode ?? null, source)}{" "}
              {condLabel || "พยากรณ์"}
            </div>
            {(tempMax != null || tempMin != null) && (
              <div className="sm:text-[16px] text-slate-500 flex gap-2 mt-0.5">
            ...
            <div className="flex min-w-max px-2 py-2 gap-0.5 h-full items-center">
              {upcoming.map((s, idx) => {
                ...
                return (
                  <div
                    key={s.time}
                    className={`flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-lg min-w-12 text-center ${
                      isCurrent
                  ...
                  >
                    <span
                      className={`font-semibold ${isCurrent ? "text-sky-600" : "text-slate-500"}`}
                    >
                      {isCurrent
                        ? "ตอนนี้"
                    ...
                    <span className="text-base leading-none">
                      {condIcon(s.weatherCode, source)}
                    </span>
                    <span
                      className={`font-bold ${isCurrent ? "text-slate-800" : "text-slate-700"}`}
                    >
...
function TodayForecastCard({ tmd }: { tmd: TmdForecastDay[] }) {
  ...
  return (
    <Card className="bg-sensor-rain-bg border-sensor-rain-border shadow-sm border h-full">
      <CardHeader className="flex flex-row items-center justify-between pb-1 pt-3 px-3">
        <CardTitle className="font-semibold text-muted-foreground uppercase tracking-wide leading-none">
          พยากรณ์วันนี้ <span className="font-normal">(กรมอุตุฯ)</span>
        </CardTitle>
        <CloudRain
          className="h-4 w-4 text-blue-600 opacity-80"
          aria-hidden="true"
        />
      </CardHeader>
      <CardContent className="px-3 pb-2">
        {todayFc ? (
          <div className="flex flex-col gap-1 text-xs">
            {[
              {
                icon: <Thermometer className="h-3 w-3 shrink-0" />,
                label: "อุณหภูมิ",
                value:
                  todayFc.maxTemp != null ? (
                    <>
                      <span className="text-red-500">
                        {todayFc.maxTemp.toFixed(1)}
                      </span>
                      <span className="text-muted-foreground mx-0.5">/</span>
                      <span className="text-blue-500">
                        {todayFc.minTemp?.toFixed(1) ?? "—"}
                      </span>
              ...
              {
                icon: <CloudRain className="h-3 w-3 shrink-0" />,
                label: "ฝนสะสม",
                value: <span>{todayFc.totalRain?.toFixed(1) ?? "—"} mm</span>,
              },
              {
                icon: <Droplets className="h-3 w-3 shrink-0" />,
                label: "ความชื้น",
                value: <span>{todayFc.avgHumidity?.toFixed(0) ?? "—"}%</span>,
              },
              {
                icon: <Wind className="h-3 w-3 shrink-0" />,
                label: "ลม",
                value: (
                  <span>{todayFc.avgWindSpeed?.toFixed(1) ?? "—"} m/s</span>
                ),
              },
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    loadSystemConfig().then((c) => {
      setSysConfig(c);
      if (c.dashboardRefreshSeconds !== null)
        setPollInterval(c.dashboardRefreshSeconds);
    });
  }, []);
      ...
      try {
        ...
        const merged: LiveData = soilData
          ? {
              ...
              soilTemperature1:
                data.soilTemperature1 ?? soilData.soilTemperature1,
              soilTemperature2:
                data.soilTemperature2 ?? soilData.soilTemperature2,
            }
          : data;
        console.log("Fetched live data:", merged);
        setLive(merged);
  ...
  useEffect(() => {
    ...
    getTmdForecast(selectedStationId)
      .then((r) => {
        setTmdNoKey(r.noKey);
    ...
    getHourlyForecast(selectedStationId)
      ...
      .catch(() => {});
    getTmdWarnings(selectedStationId)
      .then(setTmdWarnings)
      .catch(() => {});
  }, [selectedStationId, selectedStation?.type]);

  useEffect(() => {
    if (!selectedStationId || isGuest) return; // Guest has no camera access
    getTodayImages(selectedStationId)
      .then(setTodayImages)
      .catch(() => {});
  }, [selectedStationId, isGuest]);
    ...
    <div className="space-y-4 max-w-[1400px] mx-auto pb-8">
      ...
      <div className="flex items-end justify-between border-b pb-4 flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          >
            <RefreshCw
              className={`h-3 w-3 ${isLoading || pollingPulse ? "animate-spin text-primary" : ""}`}
              aria-hidden="true"
            />
      ...
      {!selectedStation ? (
        ...
        <>
          {/* 3. Status Bar */}
          <div className="bg-muted/50 rounded-lg p-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border shadow-sm text-[18px]">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-1.5 ml-2">
                <div
                  className={`h-2.5 w-2.5 rounded-full ${isOnline ? "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]" : "bg-red-500"}`}
                ></div>
                <span
            ...
            <div className="flex items-center gap-3">
              <span className="text-muted-foreground uppercase">อัปเดต:</span>
              <span className="font-bold">
                {live?.sensorTime
                  ? formatThaiDateTimeSeconds(live.sensorTime) + " น."
                  : "--"}
              </span>
              <div className="hidden sm:block opacity-60">
                เซ็นเซอร์: {secondsLabel(sensorAgo)}
              </div>
              {SHOW_TOR && (
          ...
          {SHOW_TOR && (
            <div className="text-[10px] font-mono text-muted-foreground/50 -mb-1">
              TOR 4.5.3.1 — ดึงข้อมูลเซนเซอร์ + คำนวณ VPD &nbsp;|&nbsp; TOR
              4.5.3.4 — แสดงหน้าจอรวมค่าต่างๆ
            </div>
          )}
          {isWeatherStation &&
            ...
            (() => {
              const todayStr = new Date().toISOString().slice(0, 10);
              const todayTmd =
                tmdForecast.find((d) => d.date === todayStr) ?? tmdForecast[0];
              return (
                <HourlyForecastCard
          ...
          {isWeatherStation ? (
            <>
              <div className="grid gap-3 grid-cols-2 sm:grid-cols-2 md:grid-cols-4 items-stretch">
                <SensorCard
                  title="อุณหภูมิ"
                  value={live?.airTemperature}
                  unit={sysConfig.conversions.airTemp.unit}
                  icon={Thermometer}
                  type="temp"
                  dbField="CAM_main.B"
                  chartKey="airTemperature"
                />
                <SensorCard
                  title="ความชื้น"
                  value={live?.relativeHumidity}
                  unit={sysConfig.conversions.humidity.unit}
                  icon={Droplets}
                  type="humid"
                  dbField="CAM_main.A"
                  chartKey="relativeHumidity"
                />
                <SensorCard
                  title="ความเข้มแสง"
                  value={live?.lightIntensity}
                  unit={sysConfig.conversions.light.unit}
                  icon={Sun}
                  type="light"
                  dbField="CAM_main.C"
                  chartKey="lightIntensity"
                />
                <SensorCard
                  title="ปริมาณน้ำฝน"
                  value={live?.rainfall}
              ...
              <div className="grid gap-3 grid-cols-2 sm:grid-cols-2 md:grid-cols-4 items-stretch">
                <WindCombinedCard
                  speed={live?.windSpeed}
                  deg={live?.windDirection}
                  dbField="CAM_main.F/H"
                />
                <SensorCard
                  title="ความกดอากาศ"
                  value={live?.atmosphericPressure}
                  unit={sysConfig.conversions.pressure.unit}
                  icon={Gauge}
                  type="pressure"
                  dbField="CAM_main.E"
                  chartKey="atmosphericPressure"
                />
                <SensorCard
                  title="VPD (ทุเรียน)"
                  value={live?.vpd}
            ...
            <>
              <div className="grid gap-3 grid-cols-2 sm:grid-cols-2 md:grid-cols-4 items-stretch">
                <SensorCard
                  ...
                  value={live?.soilMoisture1}
                  unit={sysConfig.conversions.soilMoist1.unit}
                  icon={Droplets}
                  type="soil"
                  dbField="CAM_client.A"
                  chartKey="soilMoisture1"
                />
                <SensorCard
                  title="อุณหภูมิดิน 15cm"
                  value={live?.soilTemperature1}
                  unit={sysConfig.conversions.soilTemp1.unit}
                  icon={Thermometer}
                  type="temp"
                  dbField="CAM_client.B"
                  chartKey="soilTemperature1"
                />
                <SensorCard
                  title="ความชื้นดิน 30cm"
                  value={live?.soilMoisture2}
                  unit={sysConfig.conversions.soilMoist2.unit}
                  icon={Droplets}
                  type="soil"
                  dbField="CAM_client.C"
                  chartKey="soilMoisture2"
                />
                <SensorCard
                  title="อุณหภูมิดิน 30cm"
                  value={live?.soilTemperature2}
                  unit={sysConfig.conversions.soilTemp2.unit}
                  icon={Thermometer}
                  type="temp"
                  dbField="CAM_client.D"
                  chartKey="soilTemperature2"
                />
              </div>
            </>
          )}

          {/* 5. Camera & Hourly History — hidden for Guest */}
          {!isGuest && (
          <Card className="overflow-hidden border shadow-md p-0">
            <div className="bg-muted px-4 py-5 border-b flex justify-between items-center">
              <h3 className="font-bold uppercase tracking-tight flex items-center gap-1">
                <ImageIcon className="h-5 w-5" /> ภาพกล้องสถานี
                {SHOW_TOR && (
                  <span className="font-normal text-muted-foreground/50 ml-2">
                    TOR 4.5.3.2
                  </span>
                )}
              </h3>
              ...
              <div className="flex gap-4 flex-col md:flex-row">
                ...
                <div className="md:w-1/2 shrink-0">
                  {live?.imageUrl ? (
                    <div className="relative group">
                      ...
                      <div className="absolute bottom-2 right-2 bg-black/60 text-white text-[12px] px-2 py-0.5 rounded">
                        {live.imageTime
                          ? formatThaiDateTimeSeconds(live.imageTime) + " น."
                          : "LIVE"}
                      </div>
                    </div>
                  ) : (
                ...
                <div className="md:w-1/2 flex flex-col">
                  {(() => {
                    ...
                    return (
                      <>
                        <div className="font-semibold text-muted-foreground uppercase mb-2">
                        ...
                        {daytime.length === 0 ? (
                          ...
                          <div className="grid grid-cols-3 gap-1 overflow-y-auto max-h-[320px]">
                            {daytime.map((img, i) => (
                              <div
                                key={i}
                              ...
                              >
                                ...
                                />
                                <div className="absolute bottom-0 left-0 right-0 bg-black/60 text-white text-[12px] text-center py-0.5 ">
                    ...
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="bg-muted/40 border-b text-muted-foreground uppercase font-bold">
                          ...
                          <th className="p-3 text-center text-[16px]">ทิศลม</th>
                        </tr>
                      </thead>
                          ...
                          return (
                            <tr key={d.date} className="hover:bg-muted/20">
                              ...
                              <td className="p-3 text-center text-blue-600 text-[16px]">
                                {d.avgHumidity?.toFixed(0) ?? "—"}
                              </td>
                              <td className="p-3 text-center text-indigo-600 font-bold text-[16px]">
                                {d.totalRain?.toFixed(1) ?? "—"}
                              </td>
                              <td className="p-3 text-center text-slate-600 text-[16px]">
                                {d.avgWindSpeed?.toFixed(1) ?? "—"}
                              </td>
                              <td className="p-3 text-center text-slate-700 text-[16px]">
                                {d.avgWindDir != null ? (
                                  <span className="inline-flex items-center gap-1">
                                    <svg
                                      ...
                                      className="h-10 w-10"
                                      style={{
                                        transform: `rotate(${d.avgWindDir}deg)`,
                                      }}
                                      aria-hidden="true"
                                    >
                                      <path
                                        d="M12 2 L17 12 L12 9 L7 12 Z"
                                        fill="currentColor"
                                      />
                                    </svg>
          ...
          <div className="pt-2 pb-4">
            <div className="font-bold uppercase tracking-widest text-muted-foreground/50 mb-2 px-0.5">
              ไปยัง
            </div>
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-8 gap-2">
              ...
              {(() => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="font-medium text-muted-foreground group-hover:text-foreground leading-none text-center">
                      แผนที่
                    </span>
                  </a>
                );
              ...
              {[
                ...
                >
                  ...
                  <span className="text-[16px] font-medium text-muted-foreground group-hover:text-foreground leading-none text-center">
                    {label}
                  </span>
                </Link>
              ))}
      ...
      {imageFullscreen && live?.imageUrl && (
        ...
        >
          ...
          />
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-black/70 text-white text-xs px-3 py-1.5 rounded">
            {live.imageTime
              ? formatThaiDateTimeSeconds(live.imageTime)
              : "LIVE"}{" "}
            • {selectedStationId}
          </div>
        </div>
