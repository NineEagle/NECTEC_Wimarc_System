...
const CompareLineChart = dynamic(
  () =>
    import("@/components/charts/CompareLineChart").then((m) => ({
      default: m.CompareLineChart,
    })),
  {
    ssr: false,
    loading: () => (
...
function buildWeatherMetrics(c: SystemConfig) {
  ...
  return [
    ...
    },
    {
      ...
      label: `ความชื้นสัมพัทธ์ (${u.humidity.unit})`,
      icon: Droplets,
      color1: "#14b8a6",
      color2: "#f97316",
      sensorType: "main",
      unit: u.humidity.unit,
    },
    {
      value: "vpd",
      label: "VPD (kPa)",
    ...
    },
    {
      ...
      label: `ความเข้มแสง (${u.light.unit})`,
      icon: Sun,
      color1: "#14b8a6",
      color2: "#f97316",
      sensorType: "main",
    ...
    {
      value: "windSpeed",
      label: `ความเร็วลม (${u.windSpeed.unit})`,
      icon: Wind,
      color1: "#14b8a6",
      color2: "#f97316",
      sensorType: "main",
      unit: u.windSpeed.unit,
    },
    {
      value: "windDirection",
      label: "ทิศทางลม (°)",
    ...
    {
      value: "soilMoisture1",
      label: `ความชื้นดิน 15cm (${u.soilMoist1.unit})`,
      icon: Droplets,
      color1: "#14b8a6",
      color2: "#f97316",
      sensorType: "client",
      unit: u.soilMoist1.unit,
    },
    {
      value: "soilMoisture2",
      label: `ความชื้นดิน 30cm (${u.soilMoist2.unit})`,
      icon: Droplets,
      color1: "#14b8a6",
      color2: "#f97316",
      sensorType: "client",
      unit: u.soilMoist2.unit,
    },
    {
      value: "soilTemperature1",
      label: `อุณหภูมิดิน 15cm (${u.soilTemp1.unit})`,
      icon: Thermometer,
      color1: "#14b8a6",
      color2: "#f97316",
      sensorType: "client",
      unit: u.soilTemp1.unit,
    },
    {
      value: "soilTemperature2",
      label: `อุณหภูมิดิน 30cm (${u.soilTemp2.unit})`,
      icon: Thermometer,
      color1: "#14b8a6",
      color2: "#f97316",
      sensorType: "client",
      unit: u.soilTemp2.unit,
    },
  ];
}

function CompareSensorCard({
  title,
  live1,
  live2,
  unit,
    ...
    <div className="grid grid-cols-2 gap-2 border-b py-2 last:border-0">
      <div className="flex flex-col">
        <span className="text-muted-foreground uppercase font-bold flex items-center gap-1">
          {title}
          {title === "VPD" && <VpdInfoButton />}
        </span>
        <div className="flex items-baseline gap-1">
          <span className=" font-black text-teal-600">{renderVal(v1)}</span>
          <span className="text-muted-foreground">{unit}</span>
        </div>
      </div>
      <div className="flex flex-col text-right border-l pl-2">
        <span className="text-muted-foreground uppercase font-bold opacity-0 invisible">
          {title}
        </span>
        <div className="flex items-baseline gap-1 justify-end">
          <span className=" font-black text-orange-600">{renderVal(v2)}</span>
          <span className="text-muted-foreground">{unit}</span>
        </div>
      </div>
    </div>
...
export default function ComparePage() {
  ...
  const wimarcNum = (id: string) =>
    parseInt(id.replace(/^wimarc/, ""), 10) || 0;
  ...
  const s1Group =
    allGroups.find((g) => g.base === s1Base) ||
    permittedGroups.find((g) => g.base === s1Base);
  ...
  useEffect(() => {
    if (sensorType === "main" && !bothHaveMain && bothHaveClient)
      setSensorType("client");
    if (sensorType === "client" && !bothHaveClient && bothHaveMain)
      setSensorType("main");
  ...
  const station1Id = s1Base
    ? sensorType === "client"
      ? `${s1Base}c`
      : s1Base
    : null;
  const station2Id = s2Base
    ? sensorType === "client"
      ? `${s2Base}c`
      : s2Base
    : null;
  ...
  const station1 =
    allStations.find((s) => s.id === station1Id) ||
    permittedStations.find((s) => s.id === station1Id);
  const station2 =
    allStations.find((s) => s.id === station2Id) ||
    permittedStations.find((s) => s.id === station2Id);
  ...
  const currentMetric =
    WEATHER_METRICS.find((m) => m.value === metric) ||
    visibleMetrics[0] ||
    WEATHER_METRICS[0];
  ...
  const sanitized1 = useMemo(
    () => applyLimits(readings1),
    [readings1, applyLimits],
  );
  const sanitized2 = useMemo(
    () => applyLimits(readings2),
    [readings2, applyLimits],
  );
  ...
  const mergedData = useMemo(() => {
    ...
    for (let i = 0; i < points.length; i++) {
      if (i > 0 && points[i].ts - points[i - 1].ts > GAP_MS) {
        out.push({
          ts: Math.round((points[i - 1].ts + points[i].ts) / 2),
          val1: null,
          val2: null,
        });
      }
  ...
  return (
    <div className="space-y-4 max-w-[1400px] mx-auto pb-8">
      ...
      <div className="flex items-end justify-between border-b pb-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight flex items-center gap-2">
            เปรียบเทียบ 2 สถานี
            {/* <span className="font-mono bg-muted px-1.5 py-0.5 rounded text-muted-foreground uppercase">TOR 4.5.7</span> */}
          </h1>
      ...
      {allGroups.length < 2 ? (
        <Alert>
          <AlertDescription>
            ต้องมีสถานีอย่างน้อย 2 จุด ในระบบเพื่อเปรียบเทียบ
          </AlertDescription>
        </Alert>
      ) : (
        <>
          {/* 2. Selector Bar */}
          <div className="bg-muted/50 rounded-lg p-4 space-y-4 border shadow-sm ">
            {/* Row 1: Two station dropdowns + time range + CSV */}
            <div className="grid gap-3 lg:grid-cols-[1fr_1fr_auto_auto] items-end">
              <div>
                <Label className="uppercase font-bold text-teal-700">
                  สถานีที่ 1{" "}
                  {isSingleAccess && (
                ...
                <Select
                  value={s1Base}
                  onValueChange={setS1Base}
                  disabled={isSingleAccess}
                >
                  <SelectTrigger className="h-9 bg-background border-teal-200 mt-1">
              ...
              <div>
                <Label className="uppercase font-bold text-orange-700">
                  สถานีที่ 2
                </Label>
                <Select value={s2Base} onValueChange={setS2Base}>
                  <SelectTrigger className="h-9 bg-background border-orange-200 mt-1">
                    <SelectValue placeholder="เลือกสถานี" />
                  </SelectTrigger>
                  <SelectContent>
                    {allGroups
                      .filter((g) => g.base !== s1Base)
                      .map((g) => (
                        <SelectItem
                          key={g.base}
                          value={g.base}
              ...
              <div className="flex bg-background border rounded-md p-0.5">
                {[3, 7, 15].map((d) => (
                  <button
                    ...
                    className={`px-3 py-1 font-bold rounded-sm transition-all ${timeRange === d ? "bg-teal-500 text-white shadow-sm" : "hover:bg-muted text-muted-foreground"}`}
                  >
                    {d} วัน
                  </button>
                ))}
            ...
            <div className="flex items-center gap-4 border-t pt-3">
              <Label className="uppercase font-bold text-muted-foreground shrink-0">
                ประเภท
              </Label>
              <div className="flex items-center gap-4">
                ...
                {!bothHaveMain && !bothHaveClient && (
                  <span className="text-muted-foreground italic">
                    ทั้ง 2 สถานีต้องมี sensor ประเภทเดียวกัน
                  </span>
                )}
              </div>
            </div>
                ...
                {visibleMetrics.map((m) => (
                  ...
                  >
                    <Checkbox
                      ...
                      onCheckedChange={(c) => {
                        if (c) setMetric(m.value);
                      }}
                    />
                    <m.icon className="h-3 w-3" />
                    <span className="flex items-center gap-1">
                      {m.label}
                      {m.value === "vpd" && <VpdInfoButton />}
                    </span>
                  </label>
                ))}
              </div>
          ...
          {(() => {
            const liveCards = sensorType === "client"
              ? [
                  { title: "อุณหภูมิดิน 15cm", dataKey: "soilTemperature1" as keyof LiveData, unit: sysConfig.conversions.soilTemp1.unit },
                  { title: "ความชื้นดิน 15cm", dataKey: "soilMoisture1" as keyof LiveData, unit: sysConfig.conversions.soilMoist1.unit },
                  { title: "อุณหภูมิดิน 30cm", dataKey: "soilTemperature2" as keyof LiveData, unit: sysConfig.conversions.soilTemp2.unit },
                  { title: "ความชื้นดิน 30cm", dataKey: "soilMoisture2" as keyof LiveData, unit: sysConfig.conversions.soilMoist2.unit },
                ]
              : [
                  { title: "อุณหภูมิ", dataKey: "airTemperature" as keyof LiveData, unit: sysConfig.conversions.airTemp.unit },
                  { title: "ความชื้น", dataKey: "relativeHumidity" as keyof LiveData, unit: sysConfig.conversions.humidity.unit },
                  { title: "VPD", dataKey: "vpd" as keyof LiveData, unit: "kPa" },
                  { title: "ฝน", dataKey: "rainfall" as keyof LiveData, unit: sysConfig.conversions.rain.unit },
                ]
            return (
              <div className="grid gap-4 md:grid-cols-2">
                {([{ live: live1, station: station1, color: "teal" }, { live: live2, station: station2, color: "orange" }] as const).map(({ live, station, color }, idx) => (
                  <Card key={idx} className={`shadow-sm border-t-4 border-t-${color}-500`}>
                    <CardHeader className={`py-2.5 bg-${color}-50/50 border-b flex flex-row items-center justify-between`}>
                      <CardTitle className={`font-bold uppercase text-${color}-800`}>
                        {station?.name ?? (idx === 0 ? "—" : "ยังไม่ได้เลือก")}
                      </CardTitle>
                      <Badge className={live?.lastPing ? "bg-green-500" : "bg-red-500"}>
                        {live?.lastPing ? "ONLINE" : "OFFLINE"}
                      </Badge>
                    </CardHeader>
                    <CardContent className="p-4">
                      <div className="grid grid-cols-2 gap-x-4">
                        <div className="space-y-0">
                          <CompareSensorCard title={liveCards[0].title} live1={idx === 0 ? live : null} live2={idx === 1 ? live : null} unit={liveCards[0].unit} dataKey={liveCards[0].dataKey} />
                          <CompareSensorCard title={liveCards[1].title} live1={idx === 0 ? live : null} live2={idx === 1 ? live : null} unit={liveCards[1].unit} dataKey={liveCards[1].dataKey} />
                        </div>
                        <div className="space-y-0 border-l pl-4">
                          <CompareSensorCard title={liveCards[2].title} live1={idx === 0 ? live : null} live2={idx === 1 ? live : null} unit={liveCards[2].unit} dataKey={liveCards[2].dataKey} />
                          <CompareSensorCard title={liveCards[3].title} live1={idx === 0 ? live : null} live2={idx === 1 ? live : null} unit={liveCards[3].unit} dataKey={liveCards[3].dataKey} />
                        </div>
                      </div>
                    </CardContent>
                  </Card>
            ...
            <Card className="shadow-md border overflow-hidden">
              <CardHeader className="py-3 bg-muted/20 border-b flex flex-row items-center justify-between">
                <CardTitle className=" font-bold flex items-center gap-2">
                  <Activity className="h-4 w-4" /> กราฟเปรียบเทียบ{" "}
            ...
            <Card className="shadow-md overflow-hidden border-t-4 border-t-teal-500">
              <CardHeader className="py-3 bg-muted/30 border-b flex flex-row items-center justify-between">
                <CardTitle className="font-bold uppercase tracking-tight flex items-center gap-2">
                  ตารางเปรียบเทียบค่าเฉลี่ย, สูงสุด และต่ำสุดของข้อมูล
                </CardTitle>
                  ...
                  <thead>
                    <tr className="bg-muted/50 border-b text-muted-foreground uppercase font-bold">
                      <th className="p-3 text-left">
                        ค่า ({currentMetric.label})
                      </th>
                      <th className="p-3 text-right text-teal-700">
                        {station1?.name ?? "—"}
                      </th>
                    ...
                    {[
                      ...
                      return (
                        ...
                        >
                          ...
                          <td
                            className={`p-3 text-right border-l font-black ${Math.abs(diff) > 2 ? "text-red-600 bg-red-50" : "text-slate-600"}`}
                          >
                            {diff > 0 ? "+" : ""}
                            {diff.toFixed(2)}
                          </td>
