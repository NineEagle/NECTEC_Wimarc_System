...
### 81. อุปกรณ์เสีย — ตัดวันที่ซ่อม, เพิ่ม CSV, จัดหน้าใหม่  <!-- (2026-09-04) -->

ต่อจากข้อ 80 หลังใช้งานจริงแล้วปรับ

**ตัด `fixed_date` ออก** — ตอนนี้ 1 แถวเหลือวันที่เดียวคือ `created_at` (วันที่บันทึก)
ซึ่งเป็นวันเดียวที่คนบันทึกระบุได้แม่นจริง ส่วน "วันที่เสีย" ตัดไปแล้วในรอบก่อน
startup migration `DROP COLUMN IF EXISTS` ทั้งสองคอลัมน์ (idempotent รันซ้ำได้)
ฟิลเตอร์ "เฉพาะที่ยังไม่ซ่อม" หายไปด้วยเพราะไม่มีข้อมูลให้กรองแล้ว

**เพิ่ม CSV export** (`exportFaultsToCSV` ใน `services/exportService.ts`)
เรียงคอลัมน์เหมือนตารางบนหน้าจอ: ที่ไหน → อะไรเสีย → ครั้งที่ → อาการ → เมื่อไหร่ → ใครบันทึก

**จัดหน้าใหม่ให้อ่านง่าย:**
- แถบสรุป 4 ตัวเลขด้านบน (ทั้งหมด / สถานีที่มีปัญหา / เสียซ้ำ / อุปกรณ์ที่เสียบ่อยสุด)
  ตอบว่า "มีอะไรน่าห่วงไหม" ได้โดยไม่ต้องอ่านตาราง คำนวณจากรายการที่แสดงจริงหลังกรอง
- badge "ครั้งที่" เปลี่ยนเป็นสีแดงตั้งแต่ครั้งที่ 2 ขึ้นไป — ของซ้ำคือสัญญาณที่ต้องเห็น
  จึงให้มันเป็นสิ่งเดียวในแถวที่เปลี่ยนสี ไม่งั้นตาไม่รู้จะมองตรงไหน
...
### 85. เอกสารเตรียมอบรมกรรมการ (หัวข้อวันที่ 8)  <!-- (2026-09-05) -->

เพิ่มไฟล์ `notes/TRAINING_2026-09-08.html` — เอกสารเตรียมอบรม 6 หัวข้อตามที่ผู้ว่าจ้างขอ
(หน้าเดียวจบ เปิดในเบราว์เซอร์/ฉายขึ้นจอ/สั่งพิมพ์เป็นเอกสารแจกได้ รองรับ dark mode)

เนื้อหาแบ่งเป็น 8 ส่วน พร้อมประมาณการเวลารวม ~2 ชม.:

- **00 ภาพรวม** — แผนภาพ 3 ช่องทาง (A = ESP ingest ผ่าน PHP, B = เว็บแอป JWT, C = external API key)
- **01 Hardware** — สัญญาการส่งค่าเซนเซอร์ (`wimarcID`, `A`–`H`, `src`, `age`), ตารางแมป A–H → ค่าจริง
  พร้อมสูตรทั้งสถานีอากาศ/ดิน, โค้ดอ้างอิง ESP32 ทั้งฝั่งส่งค่าและฝั่งส่งรูป (multipart `location` + `imageFile`),
  relay v2 (LoRa mutual-mirror), และเส้นทางรูปจากดิสก์ → Apache Alias → `images/latest` → หน้าเว็บ
- **02 เพิ่ม user** — 3 ทาง (หน้า `/admin/users`, สมัครเอง `/register` แล้วอนุมัติ, `POST /users` ผ่าน curl) + ตาราง RBAC
- **03 เพิ่มหน้า/ลิงก์ให้ user** — สูตร 3 ขั้น (สร้าง `app/<route>/page.tsx` → เพิ่ม `navItems` ใน AppSidebar → rebuild)
  ใช้ตัวอย่างหน้าภาพถ่ายดาวเทียม ชี้ว่า tile Esri World Imagery + CSP มีอยู่แล้ว
- **04 สร้าง weblink ใหม่** — `wimarc.in.th/mango` ไม่ต้องแตะ Apache/DNS, `PUBLIC_ROUTES` ใน AppShell,
  รายชื่อ prefix ที่ห้ามชนกับ legacy
- **05 access DB ผ่าน web app** — 2 engine (`get_db` เขียนได้ / `get_wimarc_db` อ่านอย่างเดียว),
  เส้นทางคำขอ, เช็กลิสต์ 5 จุดเวลาจะเพิ่ม field ใหม่, คำสั่ง psql สำหรับสาธิต, สรุปมาตรการความปลอดภัย
- **06 Token ไม่ผ่านหน้า login** — `POST /auth/login` → JWT (24 ชม.) ใช้กับ header ได้ทันที,
  ตารางเทียบ 3 ทางเลือก (JWT / API key `wmk_*` / ลิงก์ `?token=` ที่ยังต้องพัฒนา) พร้อมข้อจำกัดว่า
  เบราว์เซอร์ใส่ header เองไม่ได้ และ API key ไม่รับแบบ query param
- **07 เช็กลิสต์ก่อนสาธิต**

หมายเหตุที่ตรวจจากของจริงและใส่ไว้ในเอกสาร: ต้องยิง ingest ผ่าน `http://` เท่านั้น (Apache ยกเว้น path
อุปกรณ์จาก redirect HTTPS), รูปจำกัด 500,000 ไบต์และตั้งชื่อ `YYYYMMDD_HH_M.jpg` (1 รูป/ชั่วโมง ทับตัวเอง),
`location` ที่ส่งตอน upload คือ**เลขสถานี** ไม่ใช่ `wimarc_id`, และถ้าปิด `Options Indexes` ใน vhost
รูปจะหายทั้งระบบเพราะ backend หาไฟล์จาก directory listing

**commit:** ยังไม่ commit — รอเจ้าของตรวจเนื้อหาก่อน

---

### 86. ขยายหัวข้อ Hardware ในเอกสารอบรม — 2 การเปลี่ยนแปลงจริงในเฟิร์มแวร์  <!-- (2026-09-05) -->

เจ้าของระบุว่าโค้ดกล้องที่แก้รอบนี้มี 2 จุด: (1) เพิ่ม id ของกล้องตอนอัปโหลดรูป
(2) รีเซ็ตตัวนับ tick น้ำฝนทุก 10 นาทีเมื่อค่า > 0 (ถ้า = 0 ไม่ต้องทำอะไร)
จึงเขียนหัวข้อ 01 ใน `notes/TRAINING_2026-09-08.html` ใหม่ทั้งส่วน (1.1–1.7)

**หลักฐานฝั่งเซิร์ฟเวอร์ที่ค้นเจอและใช้ประกอบ:**

- **id กล้อง** — สคริปต์อัปโหลดมีวิวัฒนาการ 4 รุ่นบนดิสก์: `uploadCAMV.php`/`uploadCAMA.php`
  (ไม่มีฟิลด์บอกตัวตนเลย เขียนลง `/imageV/`, `/imageA/` โฟลเดอร์เดียว) → `uploadIMG.php`
  (`wimarcID`+`type` → `image<ID><M|C>`) → `uploadCAMVold_main.php` (`wimarcID` → `imgMain/wimarc<ID>`)
  → `uploadCAM_main.php`/`_client.php` ปัจจุบันที่ใช้ **`location`** → `imgMain|imgClient/wimarc<N>`
- โฟลเดอร์ `imgMain/wimarcUnknown_Station/` และ `imgClient/wimarcUnknown_Station/` **มีอยู่จริง**
  (สร้าง 21 มี.ค. 2569, ว่างเปล่าทั้งคู่) = เคยมีบอร์ดยิงรูปมาโดยไม่มี `location` แล้ว PHP ใช้ fallback
  `'Unknown_Station'` — ใช้เป็นตัวอย่างรูปธรรมในเอกสารว่าถ้าไม่ส่ง id จะเกิดอะไร
- **ตัวนับน้ำฝน** — เหตุผลที่ต้องรีเซ็ตอยู่ที่โค้ดฝั่งเว็บซึ่ง **บวกค่าน้ำฝนทุกแถว**:
  `app/historical/page.tsx:574` (`sum("rainfall")`) และ `services/sensorService.ts:172`
  (`rainfall.reduce((a,b) => a+b, 0)`) → ค่าต่อแถวต้องเป็นฝนในช่วง 10 นาทีนั้น ไม่ใช่ยอดสะสม
  ตัวอย่างในเอกสาร: ฝนตกจริง 3 มม. ถ้าไม่รีเซ็ต ยอดรวมรายวันจะขึ้นเป็น ~267 มม. โดยระบบไม่ error
- จังหวะรีเซ็ตควรผูกกับ `NewRecord` ที่ `wimarc_ingest_lib.php` ตอบกลับ (เงื่อนไขจริงคือ
  `$minute % 10 == 0 && $second < 30 && $G > 0`) ไม่ใช่นาฬิกาบอร์ดอย่างเดียว — ถ้าเน็ตหลุดจังหวะนั้น
  แล้วรีเซ็ตไปแล้ว ฝนช่วงนั้นหายถาวร

**ข้อเสนอแนะที่ใส่ไว้ให้คุยกับผู้ทำเฟิร์มแวร์** (ยังไม่ได้ยืนยัน): ใช้ `rainTicks -= ticks` แทน
`rainTicks = 0` กันพัลส์ที่เข้ามาระหว่างส่ง HTTP หาย · และค่า `MM_PER_TICK` ต้องเช็กกับสเปก
tipping bucket จริง เพราะเซิร์ฟเวอร์แปลงด้วยสูตรตายตัว `Rain = D / 100`

**commit:** ยังไม่ commit — รอเจ้าของตรวจเนื้อหาก่อน

---

### 87. คู่มือพัฒนาเว็บฉบับเริ่มจากศูนย์ (notes/DEV_GUIDE/)  <!-- (2026-09-08) -->

เพิ่มโฟลเดอร์ `notes/DEV_GUIDE/` — คู่มือนักพัฒนา 17 บท เขียนเป็น Markdown แยกไฟล์ต่อบท
สำหรับคนที่ไม่เคยเขียนเว็บมาก่อนเลย ให้รับช่วงพัฒนา wimarc.in.th ต่อได้เอง
(เจ้าของขอเป็น .md เพื่อนำไปจัดเป็นเล่มต่อในเครื่องอื่น — เดิมร่างเป็น HTML แล้วยกเลิก)

โครง 5 ส่วน:

- **ส่วน 1 (00–02)** ภาพรวม — 5 ชิ้นส่วนของระบบ, 3 ช่องทางข้อมูล (ESP/PHP · เว็บ/JWT · external/API key),
  ศัพท์ 20 คำ, และเหตุผลว่าทำไม `BACKEND_PROXY_URL` เป็นค่า build-time
- **ส่วน 2 (03–05)** ลงมือรัน — เครื่องมือพร้อมเวอร์ชันที่ใช้จริง (Node 20 / pnpm 9.12.3 / Python 3.11),
  ทัวร์โฟลเดอร์พร้อมตาราง "อยากแก้ X ไปที่ไหน", 3 วิธีรัน (docker / `start.sh` / `pnpm dev`)
  และการตรวจทั้งสายด้วย curl 3 คำสั่ง
- **ส่วน 3 (06–09)** อ่านโค้ดให้ออก — React เฉพาะส่วนที่โปรเจกต์นี้ใช้, เส้นทางค่าเซนเซอร์ 9 สถานี
  จาก `sensor_1min` ถึงการ์ดบนจอ, FastAPI (endpoint/Depends/RBAC 3 ชั้น), ฐานข้อมูล 2 engine
- **ส่วน 4 (10–12)** ตัวอย่างงานจริงครบวงจร — เพิ่มหน้า `/mango` + เมนู, เพิ่มค่า dew point ครบ 6 จุด
  (main.py → schemas → apiMappers → types → page → build), และทำ CRUD ทั้งชุด 9 ไฟล์
  โดยเลียนแบบระบบบันทึกอุปกรณ์เสีย
- **ส่วน 5 (13–16)** ส่งขึ้นจริง — build/deploy/rollback, ตารางอาการ-สาเหตุ-วิธีแก้,
  กฎ notes + commit ตาม CLAUDE.md, และแบบฝึกหัด 5 ชิ้นพร้อม cheat sheet

**กับดักที่ตรวจจากโค้ดจริงแล้วเขียนเตือนไว้ในเล่ม:** `_jwt_auth_middleware` ปล่อย GET ผ่านหมด
(endpoint GET ใหม่ที่ลืม `Depends(get_current_user)` = เปิดสาธารณะทันที) · `create_all()`
ไม่เพิ่มคอลัมน์ให้ตารางเดิม ต้องเขียน `ALTER TABLE ... IF NOT EXISTS` ใน `on_startup` เอง ·
`_cache` ใน `apiClient.ts` เป็นโค้ดตายแล้ว · ค่า build-time ที่แก้ใน `environment:` ไม่มีผล ·
ชื่อไอคอน lucide ผิดคือสิ่งเดียวที่ทำให้ build ล้ม (TypeScript error ไม่ล้มเพราะ `ignoreBuildErrors`) ·
`Options Indexes` ใน vhost ถ้าปิดแล้วรูปหายทั้งระบบ · offset เวลาไทย 7 ชม. ในตารางเซนเซอร์เดิม

ไม่มีรหัสผ่าน คีย์ หรือค่า secret ใด ๆ ในเล่ม ตั้งใจให้ส่งต่อให้ผู้พัฒนาใหม่ได้ทันที

**commit:** `d11205c` — docs(guide): add the from-zero developer manual

---

### 88. จัดคู่มือ DEV_GUIDE เป็นเล่ม .docx พร้อมรูปประกอบ 9 รูป  <!-- (2026-09-08) -->

เพิ่มระบบ build ที่ `notes/DEV_GUIDE/_book/` แปลง Markdown 18 ไฟล์ของ DEPLOYMENT 87
เป็นเอกสาร Word เล่มเดียว `notes/DEV_GUIDE/WiMaRC-Dev-Guide.docx` (~624 KB)
เจ้าของขอเป็น .docx เพื่อเอาไปแก้/จัดรูปเล่มต่อใน Word ได้เอง

**ไฟล์ที่เพิ่ม (ไม่แตะไฟล์บทเดิมเลย — build อ่านแล้วแปลงในหน่วยความจำ):**

- `_book/build.py` — ประกอบบท + วางรูป + เรียก pandoc + ตรวจผลลัพธ์ (`python3 build.py`)
- `_book/make_reference.py` — สร้าง `reference.docx` (ฟอนต์/ขนาด/สี/ขอบหน้า/เลขหน้า) จากค่าคงที่หัวไฟล์
- `_book/make_diagrams.py` + `_book/svgkit.py` — วาดรูป 9 รูปเป็น SVG
- `_book/metadata.yaml` — ชื่อเล่ม/ผู้จัดทำ/วันที่ สำหรับหน้าปก
- `_book/diagrams/*.svg` — รูป 9 รูป กว้าง 620 px = 16.4 ซม. พอดีความกว้างเนื้อหา A4

**รูปแบบเล่ม:** A4 ขอบ 2.5/2.0 ซม. · Sarabun 16pt · โค้ด Menlo 9.5pt (พอดีบรรทัดยาว ~81 ตัวอักษร
ซึ่งครอบคลุม 96% ของบรรทัดโค้ดในเล่ม) · โค้ดในบรรทัดข้อความ 14pt · ตาราง 14pt มีเส้นกรอบครบ ·
สารบัญ 2 ระดับเป็น TOC field ที่ Word อัปเดตเองตอนเปิด (ตั้ง `updateFields`) · เลขหน้ากลางล่าง ·
ทุกบทขึ้นหน้าใหม่อัตโนมัติผ่าน `pageBreakBefore` ในสไตล์ Heading1

**รูปประกอบ 9 รูป** — แทน ASCII เดิม 3 รูป (ภาพรวมระบบ บท 02, เส้นทางข้อมูล บท 07,
บันไดไล่ปัญหา บท 14) และวาดเพิ่มอีก 6 รูปตรงที่เนื้อหาบรรยาย flow แต่ยังไม่มีภาพ:
3 ช่องทางเข้าถึงข้อมูล (02), แผนที่โฟลเดอร์ (04), RBAC 3 ชั้น (08), ฐานข้อมูล 2 ทาง +
การแปลงรหัสสถานี (09), 6 จุดที่ต้องแก้เมื่อเพิ่มค่าใหม่ (11), build-time เทียบ runtime +
ขั้นตอน deploy/rollback (13)

> **ตั้งใจไม่วาด** ผังต้นไม้โฟลเดอร์ 66 บรรทัดในบท 04 เป็นรูป — ผังต้นไม้อ่านง่ายที่สุดในรูปแบบ
> monospace อยู่แล้ว และแก้ง่ายกว่า จึงคงเป็นบล็อกโค้ดไว้ แล้ววาด "แผนที่โฟลเดอร์" แบบย่อ
> ที่จัดกลุ่มตามหน้าที่เพิ่มให้แทน (เสริม ไม่ใช่แทนที่)

**กับดัก OOXML ที่เจอระหว่างทำและแก้ไปแล้ว:** ลำดับ element ลูกใน `<w:rPr>` / `<w:pPr>` /
`<w:style>` / `<w:sectPr>` ถูกบังคับโดย schema (เช่น `w:sz` ต้องอยู่หลัง `w:color`,
`w:pBdr` ต้องอยู่ก่อน `w:spacing`) ถ้าเรียงผิด Word จะขึ้น "Word found unreadable content"
ตอนเปิดไฟล์ — รอบแรกที่เขียนเรียงผิดหลายจุด จึงเขียน `rpr()` / `ppr()` ที่เรียงตาม schema เสมอ
และใส่การตรวจลำดับนี้ไว้ใน `build.py` ให้ตรวจทุก part ทุกครั้งที่ build

**เรื่องฟอนต์:** ต้องตั้ง `w:cs` (complex script) ด้วย ไม่ใช่แค่ `w:ascii`/`w:hAnsi`
เพราะ Word ถือว่าภาษาไทยเป็น complex script — ตั้งแค่ ascii อย่างเดียวข้อความไทยจะไม่เปลี่ยนฟอนต์
ถ้าจะเปลี่ยนไปใช้ TH Sarabun New (ฟอนต์มาตรฐานราชการ) แก้ `FONT_TH` ใน `make_reference.py` จุดเดียว

**เรื่องขนาดโค้ด:** pandoc ใส่สไตล์ syntax highlight (`KeywordTok`, `StringTok` ฯลฯ) ของมันเอง
ที่ขนาด 11pt ถ้า reference ไม่ได้ประกาศไว้ ซึ่งจะทับขนาดที่ตั้งในสไตล์ย่อหน้า `SourceCode`
จึงต้องประกาศ token ทั้ง 31 ตัวเองใน reference ให้เป็น 9.5pt

**การตรวจอัตโนมัติหลัง build:** จำนวนบทตรงกับไฟล์ต้นฉบับ · รูปครบ 9 SVG + 9 PNG สำรอง ·
มี footer · สไตล์ 6 ตัวหลักไม่ถูก pandoc เขียนทับ · XML ทุก part เรียงตรงตาม schema

**แก้เนื้อหาคู่มือที่ล้าสมัยไปพร้อมกัน 3 จุด:**

- `README.md` + บท 00 — การอ้าง `notes/TRAINING_2026-09-08.html` เขียนใหม่ให้บอกตรง ๆ ว่า
  เอกสารอบรมฮาร์ดแวร์ฉบับนั้น**ยังไม่ได้อยู่ใน git** (ตรงกับ DEPLOYMENT 86 ที่ยังไม่ได้ commit)
  ถ้าไม่มีในเครื่องให้ขอจากผู้ดูแลระบบ — เดิมเขียนเหมือนเป็นไฟล์ที่ clone มาแล้วต้องมี
- บท 00 — ตัด "(ตอนนี้ 86 รายการ)" และ "(ตอนนี้ 27 รายการ)" ออกจากรายการไฟล์ `notes/`
  เพราะตัวเลขจะล้าสมัยทุกครั้งที่มีการ note เพิ่ม เหลือแค่ชื่อไฟล์กับคำอธิบาย
- `README.md` — หัวข้อท้าย "หมายเหตุสำหรับคนที่จะเอาไปจัดเป็นเล่ม" เดิมบอกว่าแผนภาพเป็น ASCII
  และให้ไปวาดใหม่เอง ซึ่งไม่จริงแล้วหลังงานรอบนี้ เขียนใหม่เป็น "ฉบับรวมเล่ม (.docx)"
  พร้อมคำสั่ง build และตารางว่าอยากเปลี่ยนอะไรต้องแก้ไฟล์ไหน

**commit:** `e8f2ffe` — docs(guide): build the developer manual as a Word document
