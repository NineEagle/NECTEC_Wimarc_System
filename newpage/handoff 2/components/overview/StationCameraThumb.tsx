...
export function StationCameraThumb({
  ...
  return (
    <div className={`relative w-full ${aspect} ${rounded} overflow-hidden bg-muted`}>
      ...
      <span className="absolute left-1.5 top-1.5 rounded bg-black/45 px-1.5 py-0.5 font-mono text-[9px] font-semibold uppercase text-white">
        {variant === "client" ? "Client" : "Main"}
      </span>

      {showBadge && (
        <span className="absolute bottom-1.5 right-1.5 flex items-center gap-1 rounded bg-black/55 px-1.5 py-0.5 font-mono text-[9.5px] text-white">
