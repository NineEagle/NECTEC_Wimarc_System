...
const COLS = "minmax(165px,1fr) 96px 96px minmax(240px,1.6fr)"
...
export function ValidRangeSection({
  ...
  return (
    ...
    >
      <div
        className="range-head grid gap-3.5 bg-muted px-5 py-2.5 text-[11px] font-bold uppercase tracking-wide text-muted-foreground"
        style={{ gridTemplateColumns: COLS }}
      >
        <span>เซนเซอร์</span><span>Min</span><span>Max</span><span>ช่วงที่ยอมรับ (หน่วยหลังแปลง)</span>
      </div>
      {SENSORS.map((s) => {
        const lim = system.limits[s.key]
        return (
          <div key={s.key} className="range-row grid items-center gap-3.5 border-t px-5 py-3.5" style={{ gridTemplateColumns: COLS }}>
            <SensorTag s={s} />
