...
export function AppSidebar() {
  ...
  const visibleNavItems = (() => {
    if (!user) return [];
    // Guest is restricted to the dashboard only
    if (user.role === "Guest") {
  ...
  return (
    <Sidebar variant="floating" collapsible="icon">
      ...
      <SidebarHeader className="flex items-center justify-center pt-4 pb-2 px-3">
        ...
        >
          <div
            className="flex shrink-0 items-center justify-center rounded-2xl overflow-hidden bg-primary/10 shadow-sm"
          ...
          {!isCollapsed && (
            <div className="grid flex-1 text-left leading-tight min-w-0">
              <span
                className="truncate font-extrabold tracking-widest"
                style={{ fontSize: 16 }}
              >
                WIMARC
              </span>
        ...
        {isCollapsed ? (
          ...
          >
            <Avatar className="rounded-xl" style={{ width: 28, height: 28 }}>
              <AvatarFallback
                className="rounded-xl bg-primary/10 text-primary font-bold"
                style={{ fontSize: 16 }}
              >
                {initials}
              </AvatarFallback>
          ...
          <div className="rounded-2xl bg-muted/60 px-3 py-2.5 space-y-5">
            <div className="flex items-center gap-2.5">
              ...
              >
                <AvatarFallback
                  className="rounded-xl bg-primary/15 text-primary font-bold"
                  style={{ fontSize: 16 }}
                >
                  {initials}
                </AvatarFallback>
              </Avatar>
              <div className="grid flex-1 min-w-0 text-left leading-normal">
                <span
            ...
            <button
              ...
              className="flex w-full items-center justify-center gap-2 rounded-xl bg-background py-2.5 leading-normal font-medium text-muted-foreground hover:bg-destructive/10 hover:text-destructive transition-colors"
              style={{ fontSize: 16 }}
            >
